...
export class IOSDeviceDebugging {
	...
	public debug(debuggerSetup: Mobile.IDebugOnDeviceSetup): void {
		if (options["get-port"]) {
			this.$errors.fail({ formatStr: "this feature is not supported in the selected platform", suppressCommandHelp: true });
		} else if (options["start"]) {
			this.attachDebugger(debuggerSetup);
		} else if (options["stop"]) {
			this.$errors.fail({ formatStr: "this feature is not supported in the selected platform", suppressCommandHelp: true });
		} else if (options["debug-brk"]) {
			this.startAppWithDebugger(debuggerSetup);
		} else {
			this.$logger.info("Should specify exactly one option: debug-brk, start");
		}
	}

	public attachDebugger(debuggerSetup: Mobile.IDebugOnDeviceSetup): void {
		this.attachAtRuntime(debuggerSetup);
	}

	public startAppWithDebugger(debuggerSetup: Mobile.IDebugOnDeviceSetup): void {
		this.$iOSDevice.deploy(this.packageFile, this.packageName).wait();
		this.attachAtStartup(debuggerSetup);
		this.$iOSDevice.runApplication(this.packageName).wait();
	}

	private attachAtStartup(debuggerSetup: Mobile.IDebugOnDeviceSetup) {
		this.startTheFrontEnd(debuggerSetup);
	...
	private attachAtRuntime(debuggerSetup: Mobile.IDebugOnDeviceSetup): void {
		this.startTheFrontEnd(debuggerSetup);
		this.startTheBackend("AttachRequest");
	}

	private startTheFrontEnd(debuggerSetup: Mobile.IDebugOnDeviceSetup): void {
		this.openSafariFrontEnd(debuggerSetup);
	...
	private openSafariFrontEnd(debuggerSetup: Mobile.IDebugOnDeviceSetup): void {
		this.$childProcess.exec("open -a Safari " + debuggerSetup.frontEndPath).wait();
	}
